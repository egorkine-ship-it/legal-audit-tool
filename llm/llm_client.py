...
# ---------------------------------------------------------------------------
# Порядок ВАЖЕН: более длинные/специфичные фразы идут раньше коротких, а
# грамматические формы слова «нарушение» разнесены по падежам, чтобы замена не
# ломала грамматику и не создавала дублей (см. _fix_doubling ниже):
#   * «нарушения» в родительном ед.ч. (характер/факт нарушения) — единственное
#     число «признака риска»;
#   * «нарушения» во множественном — «признаки риска».
# Здесь мы не пытаемся различать падежи по контексту (это делает post-fix),
# поэтому базовые пары держим в единственном числе, а чистку дублей/грамматики
# выполняет _fix_doubling.
...
# Слова, после которых «признаки риска» стоит в родительном ед.ч. и должно быть
# «признака риска» (характер/факт/вид/форма … признака риска).
_GENITIVE_HEADS = ("характер", "характере", "факт", "факта", "факте", "вид", "вида", "форм")

# Один «квант» результата замены — фраза «признак(и) риска» в любом числе.
_RISK_PHRASE_RE = re.compile(r"призна(?:к|ки|ка|ков)\s+риска", re.IGNORECASE)


def _fix_doubling(text: str) -> str:
    """Убрать грамматические артефакты и дубли после нормализации тона.

    Известная проблема: слово «нарушения» (родительный ед.ч.) заменялось на
    множественное «признаки риска», из-за чего «характер нарушения» превращался в
    ломаное «характер признаки риска», а рядом с уже существующим «признаки риска»
    возникал дословный дубль «признаки риска признаки риска».

    Здесь мы (1) чиним падеж после родительных «якорей» (характер/факт/…),
    (2) схлопываем подряд идущие одинаковые по смыслу фразы «признак(и) риска».
    """
    if not text:
        return text
    try:
        ...
        # (1) Грамматика: «<родительный якорь> признаки риска» -> «… признака риска».
        for head in _GENITIVE_HEADS:
            # Ищем «якорь <пробелы> признаки риска» без учёта регистра.
            pattern = re.compile(
                r"(\b" + re.escape(head) + r"[а-яё]*\s+)призна(?:ки|к)\s+риска",
                re.IGNORECASE,
            )
            result = pattern.sub(lambda m: m.group(1) + "признака риска", result)

        # (2) Схлопываем подряд идущие фразы «признак(и) риска … признак(и) риска»,
        #     разделённые только пробелами, оставляя первую формулировку.
        collapsed = True
        while collapsed:
            new_result = re.sub(
                r"(призна(?:к|ки|ка|ков)\s+риска)\s+призна(?:к|ки|ка|ков)\s+риска",
                r"\1",
                result,
                flags=re.IGNORECASE,
            )
            collapsed = new_result != result
            result = new_result
...
def _sanitize_tone(text: str) -> str:
    ...
    В обоих случаях к результату применяется _fix_doubling, чтобы устранить
    грамматические артефакты и дословные дубли «признаки риска признаки риска».
    """
    if not text:
        return ""
    try:
